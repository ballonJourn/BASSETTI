...
public class DeviceInstanceEntity extends GenericEntity<String> implements RecordCreationEntity, RecordModifierEntity {
    ...
    /**
     * GeneratedValue(generator = Generators.SNOW_FLAKE): 这是 JPA（Java Persistence API）的注解，用于指定如何生成主键
     * Generators.SNOW_FLAKE  是一个自定义的主键生成策略，通常是用雪花算法（Snowflake）生成唯一的标识符
     * Schema 这是一个 Swagger 的注解，用于提供 API 文档的描述信息 描述了 id 属性的作用
     *
     * Column(name = "photo_url", length = 2048):  JPA 注解，用于指定实体属性与数据库表列之间的映射关系。
     * 指定了 photoUrl 属性在数据库表中的列名为 "photo_url"，并限制了该列的最大长度为 2048。
     *
     */
    @Override
    ...
    /**
     *
     在 Java Bean Validation 中 使用 groups 参数是为了指定哪些验证规则应该在特定的验证组（Validation Group）中生效
     通过将验证规则划分到不同的组  我们可以根据需要选择性地进行验证
     在注解 @NotBlank 中  groups = CreateGroup.class 意味着这个非空验证规则只会在 CreateGroup 这个验证组中生效
     这样的设计是为了在不同的场景中使用不同的验证规则
     */
